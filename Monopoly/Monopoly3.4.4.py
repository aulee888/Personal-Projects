...
def main():
    ...
    global player_list
    player_list = [austin, ben, priscilla]
    '''
    ...
    while len(player_list) > 1:
        for player in player_list:
            ...
            '''
            if player.prop != []:  # Statement to avoid printing just lines
                print_properties(player)

                # Option to mortgage before buying houses.
                option = input('\nMortgage/Unmortgage property? (Y/N) >>> ')
                if option.upper() in ['Y', 'YES']:
                    mortgage(player, available)

            trade_option = input('\nTrade property? (Y/N) >>> ')
            if trade_option.upper() in ['Y', 'YES']:
                trade(player)
