# Handle how advances work with turtle graphics
...
def mortgage(player, available):
    while True:
        to_mort = input('\tEnter selection by Number >>> ')
        if int(to_mort) in player.prop:
            break
        else:
            print("\t\tYou don't own that property!")
    to_mort = int(to_mort)
...
def main():
    ...
    while len(player_list) > 1:
        for player in player_list:
            ...
            for i in range(player.last_move):
                if player.move_count == 0:
                    player.turtle.forward(93.5)  # Number from end space to regular space
                    player.move_count += 1
                elif player.move_count == 9:
                    player.turtle.forward(93.5)  # Number from end space to regular space
                    player.turtle.right(90)
                    player.move_count = 0
                else:
                    player.turtle.forward(61.625)  # Number between regular spaces
                    player.move_count += 1
